...
CartLine _line(String id, num price, int qty) => CartLine(MenuItem(id: id, name: id, price: price), qty);
...
void main() {
  group('CheckoutApi', () {
    test('preview returns the breakdown net total for the cart', () async {
      final req = CheckoutRequestData(lines: [_line('m1', 30000, 2)], paymentMethod: 'CASH');
    ...
    test('submit (VietQR) returns an awaiting-payment order with a QR code', () async {
      final req = CheckoutRequestData(lines: [_line('m1', 30000, 1)], paymentMethod: 'VIETQR');
      final r = await CheckoutApi(_client()).submit(req);
