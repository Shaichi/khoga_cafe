...
-- Sửa cột thành NVARCHAR để lưu tiếng Việt
ALTER TABLE menuitems ALTER COLUMN name NVARCHAR(255);
ALTER TABLE menuitems ALTER COLUMN description NVARCHAR(255);

ALTER TABLE categorys ALTER COLUMN name NVARCHAR(255);
ALTER TABLE categorys ALTER COLUMN description NVARCHAR(255);

ALTER TABLE optiontoppings ALTER COLUMN name NVARCHAR(255);

ALTER TABLE stores ALTER COLUMN name NVARCHAR(255);
ALTER TABLE stores ALTER COLUMN address NVARCHAR(255);

-- Xóa các record cũ bị lỗi
DELETE FROM menu_item_topping_mappings WHERE menu_item_id IN (SELECT id FROM menuitems WHERE name LIKE '%?%');
DELETE FROM menuitems WHERE name LIKE '%?%';
DELETE FROM categorys WHERE name LIKE '%?%';
DELETE FROM stores WHERE name LIKE '%?%';
DELETE FROM optiontoppings WHERE name LIKE '%?%';
...
PRINT 'Cập nhật cột thành NVARCHAR và insert dữ liệu thành công!';
GO
