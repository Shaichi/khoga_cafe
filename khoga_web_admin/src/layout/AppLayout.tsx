...
export default function AppLayout() {
  ...
  const [catalogAccountOpen, setCatalogAccountOpen] = useState(false);
  ...
  const isCatalogManagement = location.pathname.startsWith('/catalog');

  const displayNavLabel = (to: string, label: string) => {
    if (!isCatalogManagement) return label;
    if (to === '/') return 'Trang chủ';
    if (to === '/catalog') return 'Thực đơn';
    if (to === '/vouchers') return 'Vouchers';
    return label;
  };

  return (
    <div className={`layout${isCatalogManagement ? ' layout--catalog-reference' : ''}`}>
      <aside className="sidebar">
          ...
          {isCatalogManagement ? (
            <>
              <button
                ...
                onClick={() => setCatalogAccountOpen((open) => !open)}
              >
                Khoga Café Admin
              </button>
              {catalogAccountOpen && (
                <div className="sidebar__catalog-account-menu">
                  <Link to="/profile" onClick={() => setCatalogAccountOpen(false)}>Hồ sơ cá nhân</Link>
        ...
        <nav className="sidebar__nav">
          {navItems.map((item) => (
            ...
            >
              {displayNavLabel(item.to, item.label)}
            </NavLink>
          ))}
        </nav>
      ...
      <div className="main">
        ...
        <main className={`content${isCatalogManagement ? ' content--catalog-reference' : ''}`}>
          <Outlet />
        </main>
      </div>
    </div>
