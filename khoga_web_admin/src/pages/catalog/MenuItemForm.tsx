...
export default function MenuItemForm() {
  ...
  const [recipe, setRecipe] =
    useState<RecipeLineState[]>([]);

  const [toppings, setToppings] =
    useState<ToppingRowState[]>([]);
  ...
  const addLocalTopping = () => {
    ...
    if (
      ...
      setError(
        'Giá cộng thêm của topping phải là số không âm.',
      );
    ...
    if (duplicated) {
      setError(
        'Topping này đã tồn tại trong danh sách.',
      );
  ...
  const addRecipeLine = () => {
    ...
    if (!material) {
      setError(
        'Vui lòng chọn nguyên liệu cần định lượng.',
      );
    ...
    if (
      ...
      setError(
        'Lượng nguyên liệu phải lớn hơn 0.',
      );
  ...
  const handleSubmit = async (
    ...
    if (invalidTopping) {
      setError(
        `Giá của topping "${invalidTopping.name}" không hợp lệ.`,
      );
    ...
    try {
      ...
      for (
        ...
        if (
          toppingPrice !==
          normalizeNumber(
            topping.initialPrice,
          )
        ) {
          await updateMenuItemTopping(
            savedItem.id,
            topping.id,
            {
              name: topping.name,
        ...
        if (
          topping.linked !==
          topping.initialLinked
        ) {
          if (topping.linked) {
            await linkMenuItemTopping(
              savedItem.id,
              topping.id,
            );
          } else {
            await unlinkMenuItemTopping(
              savedItem.id,
              topping.id,
            );
          }
  ...
  return (
    <div className="menu-edit-page">
      ...
      {error && (
        <div className="alert alert--error menu-edit-page__alert">
          {error}
        </div>
      )}
      ...
      >
        <div className="menu-edit-grid">
          <section className="menu-edit-column menu-edit-column--left">
            ...
            {/* Áp dụng ô xám trống cho cả thêm và sửa món */}

            <label className="menu-edit-field">
              <span>Danh mục</span>

              <select
                className="menu-edit-category-select menu-edit-category-select--srs-blank"
                value={categoryId}
                disabled={submitting}
                aria-label="Danh mục"
                onChange={(event) =>
          ...
          <section className="menu-edit-column menu-edit-column--right">
            <div className="menu-edit-image-section">
              <span className="menu-edit-section-label">
                Hình ảnh món ăn
              </span>
                ...
                {toppings.length ===
                  ...
                  <div className="menu-edit-topping-grid">
                    {toppings.map(
                      (topping) => (
                        <label
                          className="menu-edit-topping-card"
                          key={
                            topping.id
                          }
                        >
                          ...
                          <span className="menu-edit-topping-plus">
                            +
                          </span>
                ...
                {recipe.length ===
                  ...
                  recipe.map(
                    (line) => {
                      ...
                      return (
                        <div
                          className="menu-edit-recipe-row"
                          key={
                            line.rawMaterialId
                          }
                        >
